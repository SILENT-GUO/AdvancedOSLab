...
extern uthread_struct_t *sched_find_best_uthread(kthread_runqueue_t *kthread_runq)
{
	...
	kthread_runqueue_t *other_k_runq;
	...
	{ /* No jobs in active. switch runqueue */
		...
		if(is_kernel_thread_ready == 1 && is_credit_scheduler == 1 && is_load_balancing == 1 ){
			//in this case, there are no more uthreads to run for this kthread, activate load balancing
			for(int i = 0; i < 16 && i != current_k_ctx->cpuid; i++){
				other_k_ctx = kthread_cpu_map[i];
				if(other_k_ctx->kthread_flags & KTHREAD_DONE){ //skip the kthread if it is done
					continue;
				}
				other_k_runq = &(other_k_ctx->krunqueue);
				if(other_k_runq->active_runq->uthread_mask) {
					gt_spin_lock(&(other_k_runq->kthread_runqlock));
					if(!other_k_runq->active_runq->uthread_mask) {
						gt_spin_unlock(&(other_k_runq->kthread_runqlock));
						continue;
					}
					// printf("The mask of the other runq is %d\n", other_k_runq->active_runq->uthread_mask);
					other_runq = other_k_runq->active_runq;
					...
					gt_spin_unlock(&(other_k_runq->kthread_runqlock));
					gt_spin_lock(&(kthread_runq->kthread_runqlock));
					printf("Previous runq mask is %d\n", runq->uthread_mask);
					__add_to_runqueue(runq, u_obj);
					printf("New runq mask is %d\n", runq->uthread_mask);
					// gt_spin_unlock(&(kthread_runq->kthread_runqlock));
					
					goto find_active_obj;
				}
				
				// }else if (other_k_runq->expires_runq->uthread_mask){
				// 	gt_spin_lock(&(other_k_runq->kthread_runqlock));
				// 	other_runq = other_k_runq->expires_runq;
				// 	// /* Find the highest priority bucket */
				// 	uprio = LOWEST_BIT_SET(other_runq->uthread_mask);
				// 	prioq = &(other_runq->prio_array[uprio]);
				...
				// 	u_obj = TAILQ_FIRST(u_head);
				// 	gt_spin_unlock(&(other_k_runq->kthread_runqlock));
				// 	switch_runqueue(other_k_runq->expires_runq, &(other_k_runq->kthread_runqlock), kthread_runq->expires_runq, &(kthread_runq->kthread_runqlock), u_obj);
				// 	break;
				// }
